...
if $INPUT_HEROKU_CREATE_BACKUP || [ -z "$LAST_BACKUP_DATE" ]; then
...
if $INPUT_RAILS_MIGRATE_BLOBS; then
  # Update blobs to use staging bucket
  DATABASE_URL="$(heroku pg:credentials:url | grep -o -E -m 1 'postgres://.+')"
  psql $DATABASE_URL -c "UPDATE active_storage_blobs SET service_name='staging' WHERE service_name='production';"
